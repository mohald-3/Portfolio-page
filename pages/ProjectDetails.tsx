  ...
  return (
    <div className="min-h-screen pb-20 animate-in fade-in duration-500 bg-[#fafafa] dark:bg-[#0a0a0c] transition-colors duration-300">
      ...
      {lightboxIndex !== null && (
        ...
        >
          ...
          <div className="absolute inset-x-4 md:inset-x-10 flex justify-between items-center pointer-events-none z-[105]">
            ...
            <button className={`p-4 rounded-full bg-white/5 border border-white/10 text-white hover:bg-white/10 transition-all pointer-events-auto ${lightboxIndex === allImages.length - 1 ? 'opacity-20' : 'opacity-100'}`} onClick={(e) => { e.stopPropagation(); if(lightboxIndex < allImages.length - 1) setLightboxIndex(lightboxIndex + 1); }} disabled={lightboxIndex === allImages.length - 1}>
              <ChevronRight className="w-6 h-6" />
            </button>
          </div>
          <div className="relative max-w-[90vw] max-h-[85vh] flex flex-col items-center">
            <img src={allImages[lightboxIndex]} alt="" className="max-w-full max-h-[80vh] object-contain shadow-2xl rounded-lg" onClick={(e) => e.stopPropagation()}/>
            <div className="mt-6 text-zinc-500 font-mono text-xs uppercase tracking-[0.3em]">IMAGE {lightboxIndex + 1} / {allImages.length}</div>
          </div>
        </div>
      )}
      ...
      <div className="max-w-7xl mx-auto px-6 lg:px-12 py-24 space-y-32">
        {/* Gallery Section - Museum Style (Adaptive Width) */}
        {allImages.length > 0 && (
          <div className="grid md:grid-cols-12 gap-12 items-start">
            <div className="md:col-span-4 sticky top-24">
              <div className="inline-flex p-3 bg-purple-500/10 dark:bg-purple-900/20 rounded-xl border border-purple-500/20 mb-6"><ImageIcon className="w-6 h-6 text-purple-600 dark:text-purple-400" /></div>
              ...
              <div className="mt-8 flex items-center gap-3 text-zinc-500 font-mono text-[10px] uppercase tracking-widest">
                <MousePointer2 className="w-3 h-3 animate-pulse" /> Horizontal scroll
              </div>
            </div>
            
            <div className="md:col-span-8">
              <div className="relative">
                {/* Fixed height container, children determine their own width */}
                <div 
                  ref={scrollContainerRef} 
                  className="flex gap-8 overflow-x-auto pb-8 no-scrollbar -mx-4 px-4 h-[400px] md:h-[550px] items-center"
                >
                  {allImages.map((image, i) => (
                    <div 
                      key={i} 
                      className="flex-none h-full group cursor-zoom-in snap-start"
                      onClick={() => setLightboxIndex(i)}
                    >
                      <div className="h-full relative rounded-2xl overflow-hidden border border-zinc-200 dark:border-white/10 bg-white dark:bg-[#0a0a0c] shadow-lg transition-all duration-500 hover:border-purple-500/50 hover:shadow-2xl hover:shadow-purple-500/10">
                        <img 
                          src={image} 
                          className="h-full w-auto object-contain transition-transform duration-500 group-hover:scale-[1.01]" 
                  ...
                  <div className="flex-none w-24 h-full"></div>
                </div>
              </div>
            </div>
          </div>
